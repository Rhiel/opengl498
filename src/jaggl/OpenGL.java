...
public class OpenGL implements GLConstants {

	public static Hashtable<Thread, OpenGL> contexts = new Hashtable<>();

	public long peer;
	public Hashtable<String, String> extensions;
	public Thread j;
	...
	public boolean hasExtension(String string) {
		if (null == extensions) {
			...
			String string_5_ = glGetString(GL_EXTENSIONS);
			int i = 0;
			for (;;) {
				...
				if (string_7_.length() != 0) {
					extensions.put(string_7_, string_7_);
				}
				i = i_6_ + 1;
			}
			String string_8_ = string_5_.substring(i).trim();
			if (string_8_.length() != 0) {
				extensions.put(string_8_, string_8_);
			}
		}
	...
	public synchronized boolean tryAttach() {
		...
		if (attachPeer()) {
			OpenGL opengl_9_ = contexts.put(thread, this);
			if (null != opengl_9_) {
				opengl_9_.j = null;
			}
			j = thread;
	...
	public synchronized boolean tryDetach() {
	...
	public static native void glDrawArrays(int mode, int offset, int length);

	public static native void glDrawElements(int mode, int length, int type, long l);
